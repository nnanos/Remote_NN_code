...
def evaluation(estimate,musdb_track,target,dur=None,orthogonality_assumption=False,win=None,padding_flag=None):
    ...
    if dur:
        vocals = librosa.to_mono(musdb_track.targets['vocals'].audio[50*musdb_track.rate:(50+dur)*musdb_track.rate,:].T)[:len(estimate)]
        drums = librosa.to_mono(musdb_track.targets['drums'].audio[50*musdb_track.rate:(50+dur)*musdb_track.rate,:].T)[:len(estimate)]
        bass = librosa.to_mono(musdb_track.targets['bass'].audio[50*musdb_track.rate:(50+dur)*musdb_track.rate,:].T)[:len(estimate)]
        other = librosa.to_mono(musdb_track.targets['other'].audio[50*musdb_track.rate:(50+dur)*musdb_track.rate,:].T)[:len(estimate)]

        target_source = librosa.to_mono(musdb_track.targets[target].audio[50*musdb_track.rate:(50+dur)*musdb_track.rate,:].T)[:len(estimate)]
