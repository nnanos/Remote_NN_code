...
def valid(args, unmix, device, valid_sampler):
    ...
    with torch.no_grad():
        for x, y in valid_sampler:
            ...
            #for the tranformer: (high complexity because of the high sequence length)
            x, y = x[:,:,:x.shape[2]//2].to(device), y[:,:,:x.shape[2]//2].to(device)
            Y_hat = unmix(x)
            Y = unmix.transform(y)
...
def main():
    ...
    for epoch in t:
        ...
        '''
        #save the model's dict state
        target_path = target_path.joinpath('model_state_dict.pt')
        torch.save(unmix.state_dict(), target_path)
        '''
